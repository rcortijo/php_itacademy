...
SELECT nombre, LEFT(nombre,2) AS caracteres FROM fabricante;
...
SELECT a.nombre, MAX(a.`precio`) AS precio  FROM producto a INNER JOIN `fabricante` b ON a.`codigo_fabricante`=b.`codigo` WHERE b.`nombre`='Lenovo' GROUP BY b.`nombre`;

#39.Lista el nombre del producto más barato del fabricante Hewlett-Packard.
SELECT a.nombre, MIN(a.`precio`) AS precio  FROM producto a INNER JOIN `fabricante` b ON a.`codigo_fabricante`=b.`codigo` WHERE b.`nombre`='Hewlett-Packard' GROUP BY b.`nombre`;
...
SELECT a.apellido1 AS primer_apellido, a.apellido2 AS segundo_apellido, a.nombre FROM `persona` a INNER JOIN `alumno_se_matricula_asignatura` b ON a.`id`=b.`id_alumno` GROUP BY b.`id_alumno`;

#2.Averigua el nombre y los dos apellidos de los alumnos que no han dado de alta su número de teléfono en la base de datos.
SELECT nombre, CONCAT(apellido1,' ',apellido2) AS apellidos FROM persona WHERE telefono = '' OR telefono IS NULL;

#3.Devuelve el listado de los alumnos/as que nacieron en 1999.
SELECT nombre, CONCAT(apellido1,' ',apellido2) AS apellidos FROM persona WHERE YEAR(`fecha_nacimiento`)='1999';
...
SELECT a.`nombre`,b.`anyo_inicio`,b.`anyo_fin` FROM `asignatura` a INNER JOIN `curso_escolar` b ON a.`curso`=b.`id` INNER JOIN `alumno_se_matricula_asignatura` c ON c.`id_curso_escolar`=b.`id` INNER JOIN `persona` d ON d.`id`=c.`id_alumno` WHERE d.`nif`='26902806M' GROUP BY c.`id_alumno`;
...
SELECT a.* FROM `persona` a INNER JOIN `alumno_se_matricula_asignatura` b ON a.`id`=b.`id_alumno` WHERE b.`id_curso_escolar`='5' GROUP BY a.`id`;
...
SELECT c.`nombre` AS nombre_departamento,a.`apellido1` AS primer_apellido,a.`apellido2` AS segundo_apellido,a.`nombre` FROM persona a INNER JOIN profesor b ON a.`id`=b.`id_profesor` LEFT JOIN departamento c ON c.`id`=b.`id_departamento` WHERE b.`id_departamento` IS NULL;

#2.Devuelve un listado con los profesores/as que no están asociados a un departamento.
SELECT b.`id_profesor`,a.`nombre`,a.`apellido1`,a.`apellido2` FROM persona a INNER JOIN profesor b ON a.`id`=b.`id_profesor` LEFT JOIN departamento c ON c.`id`=b.`id_departamento` WHERE b.`id_departamento` IS NULL;
...
SELECT pe.`apellido1`,pe.`apellido2`,pe.`nombre` FROM profesor a INNER JOIN persona pe ON pe.`id`= a.`id_profesor` AND pe.`tipo`='profesor' LEFT JOIN asignatura b ON a.`id_profesor`=b.`id_profesor` WHERE b.`id_profesor` IS NULL;
...
SELECT a.`id`,a.nombre FROM departamento a INNER JOIN profesor b ON b.`id_departamento`=a.`id` LEFT JOIN asignatura c ON c.`id_profesor`=b.`id_profesor` WHERE c.`curso` IS NULL GROUP BY a.`nombre`;
...
SELECT a.`anyo_inicio`, COUNT(b.`id_curso_escolar`) AS cantidad_matriculados FROM `curso_escolar` a LEFT JOIN alumno_se_matricula_asignatura b ON b.`id_curso_escolar`=a.`id` GROUP BY a.`id`;
